...
def _create_commit_status(statuses_url: str, state: States, context: str) -> None:
    """Create a commit status to mark PR state.
    ...
    r = requests.post(statuses_url, headers=_headers(), data=payload)
    r.raise_for_status()
...
def main() -> None:
    ...
    print(
        f"Created a status for latest commit of #{args.pr_num} PR with state {args.state}."
    )
